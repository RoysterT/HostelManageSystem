...
import {get, getStaffDetail, getAccommodationChangeInfoByOrder,
    confirmAccommodationChangeWithOrder, managerRejectAccommodationChange} from "@/net";
import {getUserId, getManageBuilding} from "@/views/scripts/UserUtils";
...
getIdentityList((data) => {
    ...
    document.title = "换宿确认" + " - HMS(" + userGroup.value + ")";
...
function updateInfoTable() {
    ...
    const url = '/api/dorm/building/list/' + manageBuilding.value
...
function rejectAccommodationChange(row: AccommodationChangeInfo) {
    ...
    createDialog("驳回换宿申请", "确认驳回换宿申请" + row.recordOrder + "吗？").then((res) => {
        if (res) {
            managerRejectAccommodationChange(row.recordOrder, 6, (data) => {
                console.log("data:", data);
...
function dormChangeConfirm(row: AccommodationChangeInfo) {
    confirmAccommodationChangeWithOrder(row.recordOrder, (data) => {
...
<template>
    <div class="table-container">
        ...
        <div v-if="getDataFlag && manageBuilding.value!=''" class="table-container">
            ...
            >
                ...
                <el-table-column prop="dormChangeOrder" label="操作" fixed="right" width="280" align="center"
                    ...
                    <template #default="{row}">
                        ...
                        <el-button type="success" size="small" :icon="SuccessFilled" @click="dormChangeConfirm(row)"
                                   v-if="isToConfirm && row.studentBuildingA === manageBuilding && row.progress === 2">确认</el-button>
                        <el-button type="success" size="small" :icon="SuccessFilled" @click="dormChangeConfirm(row)"
                                   v-else-if="isToConfirm && row.studentBuildingB === manageBuilding && row.progress === 3">确认</el-button>
                        ...
                        <el-button type="danger" size="small" :icon="CircleCloseFilled" @click="rejectAccommodationChange(row)"
                                   v-if="isToConfirm && row.studentBuildingA === manageBuilding && row.progress === 2">驳回</el-button>
                        <el-button type="danger" size="small" :icon="CircleCloseFilled" @click="rejectAccommodationChange(row)"
                                   v-else-if="isToConfirm && row.studentBuildingB === manageBuilding && row.progress === 3">驳回</el-button>
                ...
                <el-table-column prop="dormChangeOrder" label="操作" fixed="right" width="120" align="center" v-else>
                    <template #default="{row}">
                        <el-button type="primary" size="small" :icon="InfoFilled" @click="dormChangeInfo(row)">详情
                        </el-button>
                    </template>
                </el-table-column>
            </el-table>
    ...
    >
        <template #header="{ close, titleId, titleClass }">
            <h4 :id="titleId" :class="titleClass">换宿申请号[ {{ dormChangeInfoDrawerData.recordOrder }} ]的详情信息</h4>
        </template>
